      ...
      <div class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-3xl p-8 md:p-14 rounded-[3.5rem] shadow-2xl w-full max-w-xl border border-white/50 dark:border-zinc-800 animate-fade-in relative z-10">
        ...
        <div class="space-y-6">
           ...
           <div class="flex flex-col gap-3 pt-6">
              ...
              <button (click)="cancelLogin()" class="text-[10px] font-black text-gray-400 hover:text-gray-900 dark:hover:text-zinc-100 uppercase tracking-widest py-3 transition-colors">
                 Voltar para Vitrine
              </button>
           </div>
...
export class LoginComponent {
  ...
  async handleLogin() {
    if (!this.email || !this.password) {
      this.errorMessage.set('Credenciais obrigatórias.');
      return;
    }
    ...
    const role = this.dataService.authenticate(this.email, this.password);
